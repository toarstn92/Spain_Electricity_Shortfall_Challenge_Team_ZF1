# 3_AWS_and_Flask_webserver

Build a Web-based API for the Model using the Flask web server framework and hosted within an AWS EC2 instance, making the model available to friends, workplace, future clients, and essentially the world at large!

# Breakdown
* Fork the **[template repo](https://github.com/Explore-AI/load-shortfall-regression-predict-api)** provided on GitHub to help setup the API.

* Modify the code to enable the API serve your model developed in the first task (1_EDA_and_Data_Engineering_and_Model_Creation)

* Carefully follow the below instructions on how to setup the API on a local computer, as well as on an EC2 instance.
